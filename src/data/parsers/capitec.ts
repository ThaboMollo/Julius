...
export function parseCapitec(csvText: string): ParsedTransaction[] {
  ...
  const descIdx = findIndex(header, ['description', 'desc', 'details', 'narration'])
  const debitIdx = findIndex(header, ['debit', 'debits'])
  const creditIdx = findIndex(header, ['credit', 'credits'])
